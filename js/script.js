(function ($) {
  ...
  if ($(".countdown").length) {
    $(".countdown").timeTo({
      timeTo: new Date(
        new Date("Fri Jun 15 2018 09:00:00 GMT+0600 (Bangladesh Standard Time)")
      ),
      displayDays: 2,
      theme: "white",
    ...
    $("#maximage").maximage({
      ...
      backgroundSize: function ($item) {
        ...
        if ($.Window.data("w") / $.Window.data("h") < $item.data("ar")) {
          // Image size based on height
          $item
            .height($.Window.data("h") - verticalOffset)
            .width(
              (
                ($.Window.data("h") - verticalOffset) *
                $item.data("ar")
              ).toFixed(0)
            );
        } else {
          // Image size based on width
          $item
            .height(
              (
                ($.Window.data("w") - horizontalOffset) /
                $item.data("ar")
              ).toFixed(0)
            )
